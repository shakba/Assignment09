...
# shakedbason, 2020-Sept-07, Added code save and load
#------------------------------------------#
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        try:
            ...
            with open(file_name_track, 'w') as file:
                for disc in lst_Inventory:
                    lstObj = disc.cd_tracks 
                    discID = disc.cd_id
                    for track in lstObj: 
                        if track is not None: 
                            strIDplusTrack ='{},{}'.format(discID, track.get_record()) 
                            file.write(strIDplusTrack)
                        # file.write(disc.cd_id, track.get_record())
    ...
    def load_inventory(file_name: list) -> list:
        ...
        try:
            ...
            with open(file_name_track, 'r') as file:
                for line in file: 
                    trackData = line.strip().split(',') 
                    current = PC.DataProcessor.select_cd(lst_Inventory, trackData[0])
                    new_track = DC.Track(int(trackData[1]), trackData[2], trackData[3])
                    current.add_track(new_track) 
        except Exception as e:
            print('There was a general error!', e, e.__doc__, type(e), sep='\n')
